exports.run = (client, message, Discord, args) => {
    ...
    if(message.content == '--help') { //base help
     ...
     return message.channel.send({embed}).then(m=>m.react("⏲"))
    } else if(helpCommand === ('walk')) { 
     const embed = new Discord.RichEmbed()
        .setAuthor(`Walk`, 'https://a.safe.moe/h0JMI.png')
        .setColor('54719E')
        .setThumbnail(client.user.displayAvatarURL)
        .setFooter(`Memes | Any message from me can be removed by reacting with a ⏲ emoji.`)
        .addField(`__Description:__`, `Don't mind me, just casually strolling along with my garbage bin.`)
        .addField(`__Usage:__`, "`--walk <mention> [caption]`")
     return message.channel.send({embed}).then(m=>m.react("⏲"))
     ...
     return message.channel.send({embed}).then(m=>m.react("⏲"))
    } else if(helpCommand === ('plsshut')) { 
     const embed = new Discord.RichEmbed()
        .setAuthor(`Pls Shut`, 'https://a.safe.moe/h0JMI.png')
        .setColor('54719E')
        .setThumbnail(client.user.displayAvatarURL)
        .setFooter(`Memes | Any message from me can be removed by reacting with a ⏲ emoji.`)
     ...
     const embed = new Discord.RichEmbed()
        ...
        .addField(`__Note:__`, `This command can only be used in NSFW channels.`)
     return message.channel.send({embed}).then(m=>m.react("⏲"))
    } else if(helpCommand === ('setbong')) { //HECKING DISCORD IS BROKE ASS FUCK FUCK FUCK
        const embed = new Discord.RichEmbed()
           .setAuthor(`SetBong`, client.user.displayAvatarURL)
           .setColor('54719E')
           .setThumbnail(client.user.displayAvatarURL)
           .setFooter(`Info | Any message from me can be removed by reacting with a ⏲ emoji.`)
           .addField(`__Description:__`, `Due to Discord's new update that removed default channels, you must now manually set a channel for me to bong in. Say this in the channel that you want the bongs in.`)
           .addField(`__Usage:__`, "`--setbong`")
           .addField(`__Note:__`, `There is currently no way to reverse this, if I stop bonging, please use this command again. It is currently a temporary workaround until I find a permanent solution.`)
        return message.channel.send({embed}).then(m=>m.react("⏲"))
    } else {
        const embed = new Discord.RichEmbed()
    	    .setAuthor(`BigBen`, client.user.displayAvatarURL)
